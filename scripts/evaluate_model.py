...
def main():
    ...
    model = load_model_from_config(config, opts.ckpt_path)  # TODO: check path
    model.embedding_manager.load(opts.embedding_path)
    ...
    data_loader = PersonalizedBase(opts.data_dir, size=256, flip_p=0.0)

    images = [torch.from_numpy(data_loader[i]["image"]).permute(2, 0, 1) for i in range(data_loader.num_images)]
    images = torch.stack(images)

    sim_img, sim_text = evaluator.evaluate(model, images, opts.prompt)

    output_dir = os.path.join(opts.out_dir, prompt.replace(" ", "-"))
